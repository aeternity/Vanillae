...
async function
handle_keypairs
    (obj : {keypairs? : Array<named_keypair>})
    : Promise<void>
{
...
async function
render_keypair
    (parent_ul : HTMLElement,
     nk        : named_keypair)
//     {name      : string,
//      publicKey : Uint8Array} : named_keypair)
    : Promise<void>
{
    let name      : string     = nk.name;
    let publicKey : Uint8Array = nk.publicKey;
    // create the element
    // <li></li>
    ...
    let pubkey_str : string      = await publicKey_bytes_to_str(publicKey);
    ...
    : Promise<Uint8Array>
{
    ...
    for (let input_bytes_idx0 = 0;
             input_bytes_idx0 < input_bytes_length;
             input_bytes_idx0++)
    {
        return_bytes[input_bytes_idx0] = input_bytes[input_bytes_idx0];
    }
    // copy sha bytes
    for (let shasha4_bytes_idx0 = 0;
             shasha4_bytes_idx0 < 4;
             shasha4_bytes_idx0++)
    {
